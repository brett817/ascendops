...
```bash
pm assign-tech --work-order-id <id> --tech Sam --json
```

### Vendor Assignment
```bash
# Top-level (preferred — mirrors assign-tech UX):
pm assign-vendor --work-order-id 12345 --vendor Volt --json
pm assign-vendor --work-order-id 12345 --vendor "Volt Electric" --json
...
## Notes
- `complete` requires meld to be in PENDING_COMPLETION status. Applies to work by any in-house tech (Sam, Riley, Alex, or any future in-house assignment).
