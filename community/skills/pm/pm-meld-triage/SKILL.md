...
A meld that looks unhandled from the subject may already have a vendor reply, scheduled appointment, or regional-manager note in the thread. **Triage on thread state, not title.**
...
### Out-of-Area Melds → Regional Manager
Any meld tagged to an out-of-area property goes to the regional manager, not the standard vendor dispatch queue.

```
→ cortextos bus send-message dane normal "out-of-area meld <id> — routing to the regional manager per protocol"
```
...
- A regional-manager note marked "handled" or "scheduled"
...
Treat as **High** urgency. Route to the regional manager if out-of-area; otherwise escalate to Dane with move-in date.
