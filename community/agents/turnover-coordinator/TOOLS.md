...
| Command | What it does |
|---|---|
| `create-task "Stage 1: <unit> possession intake"` | Start day-0 clock, record possession date |
| `create-task "Stage 2: <unit> punch list"` | Build scope from inspection findings |
| `create-task "Stage 3: <unit> trade coordination"` | Sequence and track trades |
| `create-task "Stage 4: <unit> QC walk"` | Verify must-fix items with evidence |
| `create-task "Stage 5: <unit> rent-ready cert"` | Issue completion record to leasing |
| `kb-ingest ./vendor-roster.md --org $CTX_ORG --scope private` | Index vendor roster for trade routing |
| `kb-query "unit <address> turn history" --org $CTX_ORG` | Recall prior turn context for a unit |
