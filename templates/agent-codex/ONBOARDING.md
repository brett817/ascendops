...
The SOUL.md template (`${CTX_FRAMEWORK_ROOT}/templates/agent-codex/SOUL.md`) contains all 7 operational pillars. You MUST preserve every section when writing. Update only:
...
```bash
TEMPLATE=$(cat "${CTX_FRAMEWORK_ROOT}/templates/agent-codex/SOUL.md")
...
11b. **Ask about migration:**
    ...
    **END YOUR TURN.** If the user says no migration, skip to step 12. If yes, continue below.
    ...
    - Copy any custom skills: for each skill in `<old_dir>/plugins/cortextos-agent-skills/skills/` (or `<old_dir>/.claude/skills/` if migrating from a Claude-Code agent), offer to copy it into `${CTX_AGENT_DIR}/plugins/cortextos-agent-skills/skills/` and create a `~/.codex/skills/${CTX_AGENT_NAME}__<skill_name>` symlink for each
...
13. **Knowledge base setup - ALWAYS DO THIS STEP:**
    ...
    **END YOUR TURN.** Wait for the user's answers.
    ...
    Based on their answers, write memory management rules to `plugins/cortextos-agent-skills/skills/memory/SKILL.md` or a new `plugins/cortextos-agent-skills/skills/memory-management/SKILL.md`:
    ```markdown
...
if [ "$TOOLS_LINES" -lt 100 ]; then
  cp "${CTX_FRAMEWORK_ROOT}/templates/agent-codex/TOOLS.md" "${CTX_AGENT_DIR}/TOOLS.md"
fi
```
...
```bash
MISSING=""
for f in IDENTITY.md SOUL.md SYSTEM.md TOOLS.md GOALS.md USER.md MEMORY.md HEARTBEAT.md AGENTS.md; do
