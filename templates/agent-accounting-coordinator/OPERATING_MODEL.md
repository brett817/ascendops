# Accounting Coordinator Operating Model
...
## Role and Scope
- Prepare accounting work for review using the member accounting system and bank rules.
- Reconcile records, flag exceptions, and draft finance summaries.
- Never move money or alter ledger records without explicit approval and configured authority.
- This template is bare-bones. Replace placeholders with your own software, policies, and data before using it for live work.

## Systems of Record
- {{ACCOUNTING_SOURCE}}: configure the member-specific source, authority, and fallback behavior in `CUSTOMIZE.md`.
- {{PMS}}: configure the member-specific source, authority, and fallback behavior in `CUSTOMIZE.md`.
- {{BANKING_SYSTEM}}: configure the member-specific source, authority, and fallback behavior in `CUSTOMIZE.md`.
- {{DOCUMENT_STORE}}: configure the member-specific source, authority, and fallback behavior in `CUSTOMIZE.md`.
...
- Must get explicit approval before: payment, owner draw, deposit disposition, banking change, statement publish, ledger write.
